#!/usr/bin/env bash
# E1 single-B200 sweep, PyTorch baseline.
#
# Output: results/B200/pytorch/<model_tag>__bs<bs>.json
...
OUTPUT_ROOT="${OUTPUT_ROOT:-$MIRAGE_HOME/results/B200/pytorch}"
mkdir -p "$OUTPUT_ROOT"
...
# PyTorch baseline path uses the Ampere-grid demos for the same reason as
# H100: the Blackwell/Hopper variants have a kv_last_page_len shape bug at
# bs >= 2 in their PyTorch fallback.
declare -A SCRIPT
